...
pub struct HostCtx {
    ...
    pub lind_fork_ctx: Option<LindCtx<HostCtx, LindBootOptions>>,
}
...
impl LindHost<HostCtx, LindBootOptions> for HostCtx {
    fn get_ctx(&self) -> LindCtx<HostCtx, LindBootOptions> {
        self.lind_fork_ctx.clone().unwrap()
    }

    fn get_ctx_mut(&mut self) -> &mut LindCtx<HostCtx, LindBootOptions> {
